...
describe('[ingredientsSlice] тесты', () => {
  test('вызов с неизвестым экшеном', () => {
    ...
    const newState = ingredientsSliceReducer(initialState, unknownAction);
  ...
  describe('тест асинхронных экшенов', () => {
    test('тест загрузки ингредиентов', async () => {
      const getIngredientByIdSpy = jest
        .spyOn(api, 'getIngredientsApi')
        .mockResolvedValue(mockIngredients);

      await store.dispatch(getIngredients());

      const { ingredients, isIngredientsLoading } =
        store.getState().ingredients;

      expect(ingredients).toEqual(mockIngredients);

      expect(getIngredientByIdSpy).toHaveBeenCalledTimes(1);
      expect(isIngredientsLoading).toBe(false);
    });

    describe('провал загрузки ингредиентов', () => {
      test('есть ошибка', async () => {
        const initialState = {
          ...ingredientsInitialState,
          error: null
        };
  ...
  describe('тесты селекторов', () => {
    test('получение всего состояния', () => {
      ...
      const newState = getIngredientsSellector(state);

      expect(newState).toEqual(state.ingredients);
    });

    test('получение ингредиента по id', () => {
      const state = {
        ingredients: {
          ...ingredientsInitialState,
          ingredients: mockIngredients,
      ...
      const ingredientById = getIngredientById(
        state,
